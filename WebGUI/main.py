...
logs_dir = os.path.join(root_dir, 'Logs/pcan')
...
@app.get('/graph/{can_id}/{signal}')
def graph_data(signal: str, can_id: str, file: str = Query('log.csv')):
    """
    Example: http://10.9.141.1:8000/graph/A5/INV_Motor_Speed/?file=your_log.csv
    """
    ...
    signal_data = datagrapher.extract_from_csv(filename, signal, can_id_int)
    if not signal_data:
        return {'error': f'No data found for {signal} on CAN ID {can_id}'}
    ...
    ax.set_title(f'{signal} (Max={vmax:.2f}, Min={vmin:.2f})')
    ax.set_facecolor('#1a1a23')
    ax.set(xlabel='Timestamp (s)', ylabel=signal)
